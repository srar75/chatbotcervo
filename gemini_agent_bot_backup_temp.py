...
class GeminiAgentBot:
    ...
    def __init__(self):
        ...
        # Usar Gemini 2.5 Flash (rápido y eficiente)
...
- FECHAS: La fecha actual es 2026-02-02 (2 de febrero de 2026). REGLAS IMPORTANTES:
  ...
  * Formato de fecha para RESPONDER AL USUARIO: SIEMPRE usa DD/MM/AAAA (ejemplo: 07/02/2026). NUNCA uses el formato con guiones para el usuario.
...
Si es viaje de IDA Y VUELTA, sigue este flujo:
  1️⃣ Recopilar datos en orden: origen, destino, fecha ida, tipo viaje, FECHA REGRESO (inmediatamente), pasajeros
  2️⃣ BUSCAR vuelos de IDA (search_flights con trip_type="round_trip")
  3️⃣ Mostrar vuelos de IDA al usuario
  4️⃣ ESPERAR que el usuario SELECCIONE un vuelo de IDA
  5️⃣ DESPUÉS de seleccionar IDA → BUSCAR vuelos de VUELTA (search_flights con is_return=True)
    ...
    def handle_message(self, phone: str, message: str, media_url: str = None):
        ...
        try:
            ...
            if message_lower in ['cervo ai', 'cervo agent', 'agente cervo']:
                ...
                self._send_response(phone, welcome, session)
                return None
            ...
            if message_lower in ['salir', 'exit', 'bye', 'adios', 'chao', 'cerrar']:
                session.deactivate()
                self._send_response(phone, "👋 *¡Hasta pronto!*\n\n✨ Fue un placer ayudarte. Escribe 'cervo ai' cuando necesites viajar de nuevo.", session)
                return None
            
            # Procesar mensaje con Gemini (Gemini controla el flujo)
            self._process_with_ai(session, phone, message, media_url)
            return None
        except Exception as e:
            logger.error(f"ERROR: {str(e)}", exc_info=True)
            self._send_response(phone, "😅 Disculpa, tuve un problema técnico. ¿Podrías repetir tu solicitud?", session)
            return None

    def _process_with_ai(self, session, phone, message, media_url=None):
        """Procesa el mensaje usando Gemini 3 Pro"""
        try:
            # INTERCEPCIÓN DE OPCIÓN MANUAL VS CLARO
            # Si estamos esperando datos del pasajero (después de confirmar vuelo) pero aún no estamos en medio de un campo específico
            if session.data.get('awaiting_flight_confirmation') and not session.data.get('waiting_for_field'):
                msg_lower = message.strip().lower()
                
                # Opción 2: Manual (Texto, "manual", opción 2, emojis de escritura, o si empieza a escribir nombre)
                if any(x in msg_lower for x in ['manual', 'opcion 2', 'opción 2', 'escribir', 'texto', 'mano', '✍']) or msg_lower == '2':
                    ...
                    return self._send_response(phone, "✍️ *INGRESO MANUAL*\n\n👤 Por favor, escribe el *NOMBRE* (Primer nombre):", session)
                
                # Opción 1: Foto (Cámara, "foto", opción 1, emojis de cámara)
                elif any(x in msg_lower for x in ['foto', 'imagen', 'cedula', 'pasaporte', 'camara', 'opcion 1', 'opción 1', '📸', '📷']) or msg_lower == '1':
                     # Solo confirmar que esperamos la foto, no hacemos nada más hasta recibirla
                     return self._send_response(phone, "📸 *Excelente.*\n\nPor favor envía la foto de la *CÉDULA* o *PASAPORTE* ahora.", session)
                
                # Si el usuario simplemente escribe un nombre (asumimos manual si tiene > 3 letras y no es comando)
                elif len(msg_lower) > 3 and not media_url:
                     # Asumir que empezó a escribir el nombre
                     logger.info("Usuario empezó a escribir nombre directamente")
                     session.data['using_document_image'] = False
                     session.data['extracted_data'] = {}
                     session.data['extracted_data']['nombre'] = message.strip().upper()
                     session.data['waiting_for_field'] = 'apellido'
                     return self._send_response(phone, f"✅ Nombre guardado: {message.strip().upper()}\n\n👤 ¿Cuál es el *APELLIDO*?", session)
            ...
            if media_url and session.data.get('awaiting_flight_confirmation') and not session.data.get('waiting_for_field'):
                ...
                wati_service.send_message(phone, "📸 *Imagen recibida.*\n\n🔄 Extrayendo datos del documento, un momento por favor...")
...
Escribe "manual" para ingresar los datos manualmente."""
                        ...
                        wati_service.send_message(phone, f"✅ *Tengo los datos de los {total_passengers} pasajeros.*\n\n🎫 *Creando tu reserva...*\n\n⏳ Un momento por favor...")
            ...
            if waiting_for_field:
                ...
                if waiting_for_field == 'nombre':
                    ...
                    # Detección mejorada de Venezolano/a
                    is_venezuelan = False
                    ...
                    if is_venezuelan:
                        ...
                        session.data['waiting_for_field'] = 'cedula'
                        # Permitir Cédula o Pasaporte para extranjeros
                    ...
                    # Permitir letras y números (para pasaportes y cédulas extranjeras)
                    digits = re.sub(r'[^a-zA-Z0-9]', '', current_value.upper())
                    if len(digits) < 5:
                        return self._send_response(phone, "❌ Documento inválido. Intenta de nuevo:", session)
                    extracted_data['cedula'] = digits # Usamos cedula como campo genérico
                    ...
                    if sexo not in ['M', 'F', 'H', 'V']:
                        # Intentar inferir
                        if 'hombre' in message.lower() or 'masculino' in message.lower(): sexo = 'M'
                        elif 'mujer' in message.lower() or 'femenino' in message.lower(): sexo = 'F'
                        else: return self._send_response(phone, "❌ Por favor responde solo M (Masculino) o F (Femenino):", session)
                    
                    if sexo in ['H', 'V']: sexo = 'M'
                    extracted_data['sexo'] = sexo
                    session.data['extracted_data'] = extracted_data
                    session.data['waiting_for_field'] = 'direccion'
                    return self._send_response(phone, "✅ Sexo guardado.\n\n🏠 ¿Cuál es tu *DIRECCIÓN*?", session)

                elif waiting_for_field == 'direccion':
                    if len(message.strip()) < 5:
                        return self._send_response(phone, "❌ La dirección es muy corta. Por favor sé más específico:", session)
...
Escribe "manual" para ingresar los datos manualmente."""
                        ...
                        if booking_result.get('success'):
                            ...
                            # Verificar si hay múltiples PNR (aerolíneas diferentes)
                            multiple_pnr = booking_result.get('multiple_pnr', False)
                            
                            if multiple_pnr:
                                # DOS PNR - Aerolíneas diferentes
...
👥 *PASAJEROS ({total_passengers})*"""
                            ...
                            # Agregar cada pasajero al mensaje
                            for idx, pax in enumerate(all_passengers, 1):
                                response += f"""
...
📍 *Ruta:* {selected_flight.get('origin', 'N/A')} → {selected_flight.get('destination', 'N/A')}
📅 *Fecha:* {selected_flight.get('date', 'N/A')}
...
📍 *Ruta:* {return_flight.get('origin', 'N/A')} → {return_flight.get('destination', 'N/A')}
📅 *Fecha:* {return_flight.get('date', 'N/A')}
...
"""
   ...
   🎫 VUELTA: *{booking_result.get('pnr_vuelta')}*"""
                            ...
                            if "disponible" in error_lower or "availability" in error_lower or "no seats" in error_lower:
                                ...
                                # Solo si es error de sesión, no de ticket time limit (que es normal)
            ...
            # Solo si hay precios en sesión (momento de selección de clase)
            has_prices = session.data.get('flight_classes_prices') or session.data.get('return_flight_classes_prices')
            if has_prices:
                import re
                # Buscar patrón de clase (ej: "Clase B", "b", "clase y")
                # Solo una letra o "clase" + letra
                class_match = re.search(r'^(?:CLASE\s+)?([A-Z])$', message.upper().strip())
                
                if class_match:
                    selected_class = class_match.group(1)
                    
                    # Determinar si es ida o vuelta basado en variables de sesión
                    # Si tenemos return_flight_classes_prices y flight_confirmed en True, es vuelta.
                    # O más simple: si return_flight_classes_prices existe, estamos en vuelta (porque se obtienen después de confirmar ida)
                    is_return_flow = bool(session.data.get('return_flight_classes_prices'))
                    
                    # Validar si la clase existe en los precios
                    prices_dict = session.data.get('return_flight_classes_prices') if is_return_flow else session.data.get('flight_classes_prices')
                    
                    if prices_dict and selected_class in prices_dict:
                        logger.info(f"Interceptando selección de clase manual: {selected_class}")
...
Responde *SÍ* para continuar con los datos de pasajeros o *NO* para cambiar."""
                ...
                types.Tool(
                    function_declarations=[
                        types.FunctionDeclaration(
                            name="search_flights",
                            description="Busca vuelos de IDA disponibles entre dos ciudades en una fecha específica. Para vuelos de ida y vuelta, llama esta función dos veces (ida y vuelta por separado).",
                            parameters={
                                "type": "object",
                                "properties": {
                                    "origin": {
                                    ...
                                    "num_passengers": {
                                        "type": "integer",
                                        "description": "Número de pasajeros para el vuelo. OBLIGATORIO: Debes pregúntalo antes de buscar."
                                    }
                                },
            ...
            # Si llegamos aquí, intentar enviar la respuesta como texto simple
            # o pedir al usuario que sea más específico de forma amigable
            try:
                        ...
                        if candidate.content and candidate.content.parts:
                            for part in candidate.content.parts:
                                if hasattr(part, 'text') and part.text:
                                    return self._send_response(phone, part.text, session)
                
                # Si no hay respuesta, pedir más contexto de forma amigable
                logger.warning("No se pudo obtener respuesta de texto del modelo")
                return self._send_response(phone, "Entendido. ¿Podrías darme un poco más de detalle para ayudarte mejor?", session)
            except:
                return self._send_response(phone, "Entendido. ¿En qué más puedo ayudarte?", session)
        except Exception as e:
            error_str = str(e)
            logger.error(f"Error procesando con AI: {error_str}", exc_info=True)
            if '503' in error_str or 'overloaded' in error_str.lower() or 'UNAVAILABLE' in error_str:
                return self._send_response(phone, "⚠️ El servicio de IA está temporalmente sobrecargado. Por favor, intenta de nuevo en unos segundos.", session)
                ...
                return self._send_response(phone, "😅 Tuve un problema procesando tu solicitud. ¿Podrías intentar de nuevo?", session)
    def _handle_function_call(self, session, phone, response, history):
        ...
        try:
            ...
            if function_name == "search_flights":
                ...
                num_passengers = function_args.get('num_passengers', 1)
                
                # Guardar número de pasajeros en la sesión
                ...
                pasajeros_texto = f"👥 {num_passengers} pasajero(s)" if num_passengers > 1 else ""
                wati_service.send_message(phone, f"✈️ *Buscando los mejores vuelos para ti...*\n\n📍 Ruta: {origin} → {destination}\n📅 Fecha: {format_date_dd_mm_yyyy(date)}\n{tipo_viaje}\n{pasajeros_texto}\n\n🔍 Estoy revisando todas las opciones disponibles...")
                ...
                wati_service.send_message(phone, f"🔍 Consultando reserva {pnr}...")
                ...
                wati_service.send_message(phone, f"✈️ *Seleccionando vuelo de {flight_type}...*")
                ...
                if not flight_index:
                    ...
                    wati_service.send_message(phone, f"💰 *Consultando precios de todas las clases disponibles para vuelo de {flight_type}...*\n\n⏳ Un momento por favor...")
                    result = self._select_flight_and_get_prices_function(
                        flight_index,
                        session,
                        is_return
                ...
                wati_service.send_message(phone, "📝 *Preparando confirmación de vuelo...*")
            ...
            if structured_message and isinstance(structured_message, str):
                ...
                if any(start in structured_message for start in ['✅', '✈️', '💰', '📋', '❌', '📝']):
                    ...
                    wati_service.send_message(phone, structured_message)
                    ...
                    history.append({
                        ...
                        "parts": [{"function_response": {
                            "name": function_name,
                            "response": {"success": True, "message": "Mensaje enviado al usuario."} # Simplificado para la AI
                        }}]
                    })
                    history.append({
                        "role": "model",
                        "parts": [{"text": structured_message}] # La AI "piensa" que ella lo dijo
                    })
                    session.data['ai_history'] = history
            ...
            if 'timeout' in error_str or 'timed out' in error_str or 'connection' in error_str:
                return self._send_response(phone, "⏱️ *La búsqueda tardó demasiado*\n\nEl servidor está tardando en responder. Por favor intenta de nuevo en unos segundos.\n\n💡 *Tip:* Escribe tu solicitud de nuevo y lo intentaré.", session)
            elif '503' in error_str or 'unavailable' in error_str or 'overloaded' in error_str:
                return self._send_response(phone, "⚠️ *Servidor temporalmente ocupado*\n\nHay mucha demanda en este momento. Por favor intenta de nuevo en 30 segundos.", session)
            elif '429' in error_str or 'quota' in error_str or 'rate limit' in error_str:
                return self._send_response(phone, "⏳ *Límite de solicitudes alcanzado*\n\nPor favor espera 1 minuto e intenta de nuevo.", session)
            elif 'invalid' in error_str or 'argument' in error_str:
                return self._send_response(phone, "❌ *Datos incorrectos*\n\nParece que hay un problema con los datos ingresados. Por favor verifica la información e intenta de nuevo.", session)
            else:
                return self._send_response(phone, "😅 *Hubo un problema con la búsqueda*\n\nPor favor intenta de nuevo. Si el problema persiste, intenta con otra fecha o ruta.", session)
    def _search_flights_function(self, origin, destination, date, session, trip_type='ida'):
...
Responde *SÍ* para continuar o *NO* para cambiar."""
                ...
                if ida_flights and ida_index >= 1 and ida_index <= len(ida_flights):
                    ...
                    return {
                        ...
                        "moneda": "USD",
                        "message": f"Resumen de tu viaje IDA Y VUELTA. Por favor confirma si deseas proceder con la reserva. Responde 'SÍ' para confirmar o 'NO' para cambiar."
                    }
            ...
            if passengers_list and len(passengers_list) > 0:
                ...
                for pax in passengers_list:
                    ...
                    passenger = {
                        ...
                        'type': 'ADT',
                        'nationality': pax.get('nacionalidad', 'VE'),
                        'documentType': 'CI',
                    ...
                    # DETERMINAR TIPO DE DOCUMENTO Y PREFIJO
                    # Prioridad: pasaporte > cedula
                    if pax.get('pasaporte'):
                        # Es pasaporte
                        passenger['documentType'] = 'P'
                        passenger['idNumber'] = re.sub(r'[^a-zA-Z0-9]', '', str(pax.get('pasaporte', '')))
                    else:
                        # Es cédula - determinar V o E según nacionalidad
                        passenger['idNumber'] = re.sub(r'[^0-9]', '', str(pax.get('cedula', '')))
                        if pax.get('nacionalidad') == 'VE':
                            passenger['documentType'] = 'V'
                        else:
                            passenger['documentType'] = 'E'

                    all_passengers.append(passenger)
                    logger.info(f"Pasajero agregado: {first_name} {last_name}")
            else:
