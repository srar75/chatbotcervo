    ...
    def _process_with_ai(self, session, phone, message, media_url=None):
        ...
        try:
            ...
            if session.data.get('awaiting_flight_confirmation') and not session.data.get('waiting_for_field'):
                ...
                # Detección rápida por keywords (para velocidad)
                ...
                # Opción 1: Foto
                elif any(x in msg_lower for x in ['foto', 'imagen', 'cedula', 'cédula', 'pasaporte', 'camara', 'cámara', 'opcion 1', 'opción 1', '📸', '📷']) or msg_lower == '1':
                ...
                # No tiene documento
                    ...
                    detected_option = 'manual'  # Sin doc → manual
                else:
                    # FALLBACK: Usar AI para clasificar la intención
                    logger.info(f"Clasificando intención con AI: '{message}'")
                    ai_option = self._classify_with_ai(
                        message,
                        "El usuario debe elegir cómo ingresar los datos del pasajero para una reserva de vuelo. Las opciones son: enviar una FOTO de su documento (cédula/pasaporte), o ingresar los datos MANUALMENTE escribiendo.",
                        {
                            'foto': 'El usuario quiere enviar una foto/imagen de su documento de identidad (cédula, pasaporte)',
                            'manual': 'El usuario quiere escribir/dictar los datos manualmente, o indica que no tiene documento, o es un niño/bebé sin documento',
                        }
                    )
                ...
                if detected_option == 'manual':
                    ...
                    # AI no pudo clasificar, repetir opciones amigablemente
...
Escribe "manual" para ingresar los datos manualmente."""
                    ...
                    # Tenemos todos los pasajeros, crear reserva
                    wati_service.send_message(phone, f"✅ Fecha guardada. {pax_label}\n\n✅ *Tengo los datos de los {total_passengers} pasajeros.*\n\n🎫 *Creando tu reserva...*\n\n⏳ Un momento por favor...")
