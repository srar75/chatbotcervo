...
🖼️ **CAPACIDADES DE VISIÓN:**
ERES UN AGENTE MULTIMODAL. PUEDES VER, ANALIZAR Y ENTENDER IMÁGENES.
Si el usuario te envía una foto (como una cédula, pasaporte o cualquier imagen), PROCESALA y responde según el contenido.
NO DIGAS "Soy un modelo de texto". Tienes ojos digitales. ÚSALOS.

⚠️ FORMATO: NO USES LÍNEAS HORIZONTALES (--- o ___) NI SEPARADORES GRÁFICOS EN TUS RESPUESTAS.
...
- FECHAS: REGLAS IMPORTANTES:
  * La fecha actual se te proporcionará SIEMPRE al final de las instrucciones. Úsala como referencia absoluta.
  * Si el usuario dice "7 de febrero", "febrero 7", "7/2" SIN año, asume el año actual o el siguiente si la fecha ya pasó.
  * Si dice "mañana", calcula desde la FECHA ACTUAL.
  * Si dice "el jueves", "el próximo jueves", "jueves de la semana que viene": CALCULA LA FECHA EXACTA basándote en la FECHA ACTUAL.
  * NUNCA preguntes la fecha si el usuario ya la mencionó en un mensaje anterior (ej: "vuelo para el jueves 15"). Búscala en el historial.
  * Formato de fecha para la función search_flights: YYYY-MM-DD (ejemplo: 2026-02-15)
...
Si es viaje de IDA Y VUELTA, sigue este flujo ESTRICTAMENTE:
  ...
  2️⃣ BUSCAR vuelos de IDA (search_flights con trip_type="ida", is_round_trip=True, return_date=FECHA_REGRESO_SI_LA_TIENES)
  ...
  8️⃣ Usuario confirma ("Sí") -> :
       - SI YA TIENES LA FECHA DE REGRESO (porque el usuario la dio al principio): USA ESA FECHA AUTOMÁTICAMENTE para buscar la vuelta. NO LA VUELVAS A PEDIR.
       - SI NO TIENES LA FECHA DE REGRESO: El sistema la pedirá automáticamente.
  9️⃣ BUSCAR vuelos de VUELTA (search_flights con trip_type="vuelta", date=FECHA_REGRESO, is_round_trip=True)
...
🚨 NUNCA VUELVAS A PREGUNTAR UN DATO QUE YA DETECTASTE:
  ...
  - Si detectaste la fecha de regreso en el primer mensaje → NUNCA la vuelvas a preguntar.
...
🔴 EJEMPLO 5 (CRÍTICO - MEMORIA DE FECHA):
Usuario: "Vuelo a Margarita el próximo jueves"
Bot: "Entendido. ¿Es Solo Ida o Ida y Vuelta?"
Usuario: "Solo ida"
✅ CORRECTO: [LLAMA A search_flights INMEDIATAMENTE USANDO LA FECHA DEL JUEVES]
❌ INCORRECTO: "¿Para qué fecha deseas viajar?" (ERROR GRAVE: OLVIDASTE QUE YA DIJO "EL PRÓXIMO JUEVES")
⚠️ REGLA: SIEMPRE revisa el historial de mensajes anteriores para encontrar la fecha.

⚠️ SIEMPRE LEE EL MENSAJE ACTUAL Y EL HISTORIAL COMPLETO ANTES DE RESPONDER
⚠️ SI DETECTAS UNA CIUDAD O FECHA, GUÁRDALA Y NO LA VUELVAS A PREGUNTAR
...
⚠️ REGLAS DE DOCUMENTACIÓN (IMPORTANTE):
...
⚠️ DATOS A PEDIR SEGÚN EL VUELO:
...
⚠️ Pide los datos UNO POR UNO. No pidas todo en un solo mensaje.
...
Responde SOLO la clave (ejemplo: {list(options.keys())[0]}). Si no coincide con ninguna opción claramente, responde: NONE"""
    ...
    def _process_with_ai(self, session, phone, message, media_url=None):
        ...
        try:
            ...
            if session.data.get('awaiting_flight_confirmation') and not session.data.get('waiting_for_field'):
                ...
                if detected_confirm == 'si':
                    ...
                    if selected_class:
                        ...
                        if is_round_trip and not ida_class_confirmed:
                            ...
                            if return_date:
                                ...
                                confirm_msg = f"✅ *¡Confirmado!* Has seleccionado el vuelo de ida de *{ida_airline} {ida_num}* en clase *{ida_class_code}* para el *{ida_date_fmt}*.\n\n🔄 Ahora buscaré las opciones para tu regreso el {format_date_dd_mm_yyyy(return_date)}..."
                                ...
                                return self._send_response(phone, "✅ *Vuelo de IDA confirmado.*\n\n🔄 Ahora vamos a buscar tu vuelo de REGRESO. ¿Para qué fecha deseas volver?", session)
                        ...
                        # Si es solo ida O ya confirmamos la clase de ida (confirmación final de vuelta/resumen)
                            ...
                            return self._send_response(phone, f"{msg_confirm_text}\n\nPara completar la reserva, necesito los datos de cada pasajero. ¿Cómo prefieres ingresarlos?\n\n📸 *Opción 1:* Envía una *FOTO* de tu *{doc_label}*\n✍️ *Opción 2:* Escribe *MANUAL* para ingresar los datos a mano", session)
                    ...
                    # CASO 2: NO ha seleccionado clase aún, es la confirmación del VUELO (antes de ver clases)
                        ...
                        if pending_index:
                            ...
                            self._send_response(phone, "💰 *Consultando precios de todas las clases disponibles...*\n\n⏳ Un momento por favor...", session)
                            ...
                            if result.get('success'):
                                ...
                                if structured_message:
                                    ...
                                    if economy_classes or business_classes or first_classes:
                                        ...
                                        classes_msg += f"✈️ {result.get('aerolinea', '')} {result.get('vuelo', '')}\n"
                                        ...
                                        classes_msg += "\n━━━━━━━━━━━━━━━━━━━━━━\n\n"
                                        classes_msg += "✍️ *Escribe la letra de la clase que deseas* (ej: W, Y, B...)"
                                        
                                        return self._send_response(phone, classes_msg, session)
                                    else:
                                        return self._send_response(phone, structured_message, session)
                                else:
                                    return self._send_response(phone, "✅ Vuelo confirmado. Por favor selecciona una clase.", session)
                            else:
                ...
                if session.data.get('flight_selection_fully_confirmed') or detected_option:
                    ...
                    if detected_option == 'manual':
                        ...
                        instruction = "Por favor envía la foto de tu *CÉDULA* o *PASAPORTE* ahora."
                        
                        return self._send_response(phone, f"📸 *Excelente.*\n\n{instruction}", session)
                        ...
                        return self._send_response(phone, f"No entendí tu respuesta. Por favor elige una opción:\n\n📸 *Opción 1:* Envía una {doc_desc}\n✍️ *Opción 2:* Escribe *MANUAL* para ingresar los datos a mano", session)
            ...
            if media_url and (session.data.get('awaiting_flight_confirmation') or session.data.get('using_document_image') or session.data.get('flight_selection_fully_confirmed')) and not session.data.get('waiting_for_field'):
...
📸 *OPCIÓN 1 (RECOMENDADA):*
Envía una foto de la *CÉDULA* (Venezolanos) o del *PASAPORTE* (Extranjeros) del pasajero {current_passenger_count + 1}.
...
Escribe "manual" para ingresar los datos manualmente."""
                        ...
                        self._send_response(phone, f"✅ *Tengo los datos {pax_txt}.*\n\n🎫 *Creando tu reserva...*\n\n⏳ Un momento por favor...", session)
            ...
            if waiting_for_field:
                ...
                if waiting_for_field == 'nombre':
                    ...
                    if is_venezuelan:
                        ...
                        if is_international:
                            ...
                            if extracted_data.get('pasaporte'):
                                ...
                                msg = "✅ Nacionalidad: Venezolano.\n\n📄 ¿Qué documento vas a registrar?\n\nResponde *CÉDULA* o *PASAPORTE*"
                        else:
                            ...
                            if extracted_data.get('cedula'):
                                ...
                                msg = "✅ Nacionalidad: Venezolano.\n\n🆔 *VUELO NACIONAL:* Solo puedes viajar con tu *CÉDULA DE IDENTIDAD*.\n\nIndícame el número de *CÉDULA* (solo números):\n\n💡 *TIP:* Si es un niño/infante sin cédula, usa la del representante."
                    else:
                        ...
                        if is_international:
                            ...
                            if extracted_data.get('pasaporte'):
                                ...
                                session.data['waiting_for_field'] = 'tipo_documento_seleccion'
                                msg = "✅ Nacionalidad: Extranjero.\n\n📄 ¿Qué documento vas a registrar? (Responde CÉDULA o PASAPORTE)"
                        else:
                    ...
                    if any(x in doc_selection for x in ['CEDULA', 'CÉDULA', 'CI', 'IDENTIDAD']):
                        ...
                        msg = "✅ Seleccionaste: CÉDULA.\n\n🔢 Por favor indica el número de *CÉDULA*:\n\n💡 *TIP:* Si es un niño sin cédula, usa la del representante."
                        ...
                        msg = "✅ Seleccionaste: PASAPORTE.\n\n🛂 Por favor indica el número de *PASAPORTE*:"
                        ...
                        # Fallback con AI si no es obvio
                         ...
                         if ai_doc == 'CI':
                             ...
                             return self._send_response(phone, "❌ No entendí bien.\n\n¿Vas a registrar una *CÉDULA* o un *PASAPORTE*?", session)
                ...
                if pais_detectado:
                    ...
                    if result.get('success'):
                        ...
                        if isinstance(requisitos, str):
                            ...
                            response_text += "✈️ ¿Necesitas ayuda con algo más?"
                        else:
                            ...
                            if requisitos.get('documentos'):
                                response_text += "📄 *DOCUMENTOS:*\n"
                                for doc in requisitos['documentos']:
                                    response_text += f"• {doc}\n"
                                response_text += "\n"
                            
                            if requisitos.get('vacunas'):
                                response_text += "💉 *VACUNAS:*\n"
                                for vac in requisitos['vacunas']:
                                    response_text += f"• {vac}\n"
                                response_text += "\n"
                            
                            if requisitos.get('notas'):
                                response_text += f"⚠️ *NOTAS IMPORTANTES:*\n{requisitos['notas']}\n\n"
...
💡 Código PNR: *{result.get('pnr', pnr)}*"""
    ...
    def _select_flight_and_get_prices_function(self, flight_index, session, is_return=False):
        ...
        try:
            ...
            # Verificar si ya confirmó este vuelo anteriormente
            if is_return:
            ...
            # Si NO ha confirmado, mostrar resumen y pedir confirmación PRIMERO
...
Responde *SÍ* para ver las clases disponibles o *NO* para cambiar."""
            ...
            # Si YA confirmó el vuelo, AHORA SÍ obtener y mostrar precios de clases
   ...
   🎫 VUELTA: *{booking_result.get('pnr_vuelta')}*"""
    ...
    def _request_cedula_image_function(self, passenger_name, session):
        ...
        try:
            ...
            return {
                "success": True,
                "message": f"Para completar la reserva, necesito que envíes una foto CLARA de la *CÉDULA* (Venezolanos) o del *PASAPORTE* (Extranjeros) del pasajero. La IA extraerá automáticamente la información."
            }
        except Exception as e:
